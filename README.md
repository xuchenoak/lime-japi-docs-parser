...
解析能力：支持 class / interface / record 类型解析；record 组件按属性解析（组件注释支持 record 类级 javadoc 的 `@param` 及组件声明处两种写法），内部类、内部静态类、嵌套record 会解析并挂载到外层类的 `nestedClassNodeList`，被字段/方法/返回值引用时按全名解析（支持 `Outer.Inner` 写法）。
